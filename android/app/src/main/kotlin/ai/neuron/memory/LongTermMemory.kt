...
class LongTermMemory @Inject constructor(
    ...
    suspend fun savePreference(category: String, key: String, value: String, confidence: Float = 1.0f) {
    ...
    suspend fun getPreference(category: String, key: String): String? =
        preferenceDao.findByKey(category, key)?.value

    suspend fun getPreferencesByCategory(category: String): List<UserPreference> =
        preferenceDao.findByCategory(category)

    fun observeAllPreferences(): Flow<List<UserPreference>> =
        preferenceDao.observeAll()

    suspend fun deletePreference(category: String, key: String) {
    ...
    suspend fun getCachedWorkflow(packageName: String, taskType: String): AppWorkflow? =
        workflowDao.findByPackageAndTask(packageName, taskType)

    suspend fun getMostSuccessfulWorkflows(limit: Int = 20): List<AppWorkflow> =
        workflowDao.getMostSuccessful(limit)

    suspend fun clearAllWorkflows() = workflowDao.deleteAll()

    // --- Contacts ---

    suspend fun saveContact(displayName: String, canonicalKey: String, packageName: String) {
